...
void proximity(const std::vector<std::vector<float>>& points, int i, std::vector<bool>& isProcessed, std::vector<int>& cluster, KdTree*& tree, float distanceTol) {
	isProcessed[i] = true; 
	cluster.push_back(i); 
	std::vector<int> nearbyPoints = tree->search(points[i], distanceTol); 
	for (int idx : nearbyPoints) {
		if (!isProcessed[idx]) {
			proximity(points, idx, isProcessed, cluster, tree, distanceTol); 
		}
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i = 0; i < points.size(); i++) {
		...
		proximity(points, i, isProcessed, cluster, tree, distanceTol); 
